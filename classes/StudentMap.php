<?php
    ...
    class StudentMap extends BaseMap {
        ...
        public function findAll($ofset = 0, $limit = 30) {
            $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday, " . " gender.name AS gender, gruppa.name AS gruppa, student.num_zach, role.name AS role FROM user INNER JOIN student ON user.user_id=student.user_id " . "INNER JOIN gender ON user.gender_id=gender.gender_id INNER JOIN gruppa ON student.gruppa_id=gruppa.gruppa_id"
        ...
        public function findProfileById($id = null) {
            if ($id) {
                $res = $this->db->query("SELECT student.user_id, student.num_zach, gruppa.name AS gruppa FROM student "
                . "INNER JOIN gruppa ON student.gruppa_id=gruppa.gruppa_id WHERE student.user_id =
                $id");
