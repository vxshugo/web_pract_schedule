<?php
    ...
    class SpecialMap extends BaseMap {
        ...
        private function update(Special $special) {
            ...
            if ( $this->db->exec("UPDATE special SET name = $name, otdel_id = $special->otdel_id, active = $special->active WHERE special_id = ".$special->special_id) == 1) {
                return true;
            }
            return false;
        }
        ...
        public function findAll($ofset = 0, $limit = 30) {
            $res = $this->db->query("SELECT special.special_id, special.name, otdel.name as otdel, special.active AS special FROM special INNER JOIN otdel ON special.otdel_id = otdel.otdel_id LIMIT $ofset, $limit");
        ...
        public function findViewById($id = null) {
            if ($id) {                
                ...
                special.name, otdel.name as otdel, special.active FROM special INNER JOIN otdel ON special.otdel_id=otdel.otdel_id WHERE special_id =
                $id");
